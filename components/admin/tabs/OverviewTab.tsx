...
const OverviewTab: React.FC = () => {
  const { students, pendingApplications } = useAdminPortal();
  ...
  const metrics = useMemo(() => {
    ...
    const avgWinRate = students.length > 0 
      ? Math.round(students.reduce((sum, student) => sum + (student.stats?.winRate || 0), 0) / students.length)
      : 0;
    ...
    return [
      { title: 'Total Students', value: totalStudents.toLocaleString(), change: `+${Math.max(0, totalStudents - 1000)}%`, icon: Users },
      { title: 'Active Users', value: activeStudents.toLocaleString(), change: `+${Math.max(0, activeStudents - 500)}%`, icon: Users },
      { title: 'At-Risk Students', value: atRiskStudents.toLocaleString(), change: `-${Math.max(0, atRiskStudents)}%`, icon: AlertTriangle },
      { title: 'Pending Applications', value: pendingApps.toLocaleString(), change: `+${Math.max(0, pendingApps)}%`, icon: FileText },
      { title: 'Total P&L', value: `$${totalPnL.toLocaleString()}`, change: totalPnL >= 0 ? `+${totalPnL}%` : `${totalPnL}%`, icon: DollarSign },
      { title: 'Avg Win Rate', value: `${avgWinRate}%`, change: `${avgWinRate >= 50 ? '+' : '-'}${Math.abs(avgWinRate - 50)}%`, icon: BarChart2 },
      { title: 'Total Volume', value: totalVolume.toLocaleString(), change: `+${totalVolume}%`, icon: TrendingUp },
    ];
          ...
          return (
            <div key={index} className="bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-gray-700/50 p-6 shadow-xl hover:shadow-2xl transition-shadow">
              ...
              <div className="flex items-baseline justify-between">
                ...
                <span className={`text-sm font-semibold ${metric.change.startsWith('+') || metric.change.includes('%') && !metric.change.startsWith('-') ? 'text-green-400' : 'text-red-400'}`}>
                  {metric.change}
                </span>
              </div>
            </div>
